...
namespace NeuralNetworking
{
	public class NeuralNetwork
	{
		...
		public NeuralNetwork FeedForward(NeuralNetwork network, byte[] inputLayerBytes)
		{
			...
			double[] hiddenLayer1 = FeedNextLayer(network.inputLayer, network.hiddenLayer1, network.inputLayerWeights, network.inputLayerBiases);
			double[] hiddenLayer2 = FeedNextLayer(network.hiddenLayer1, network.hiddenLayer2, network.hiddenLayer1Weights, network.hiddenLayer1Biases);
			double[] outputLayer = FeedNextLayer(network.hiddenLayer2, network.outputLayer, network.hiddenLayer2Weights, network.hiddenLayer2Biases);
			...
			(
				...
				hiddenLayer2,
				outputLayer,
				ArrayUtils.Clone(network.inputLayerWeights),
				ArrayUtils.Clone(network.hiddenLayer1Weights),
				ArrayUtils.Clone(network.hiddenLayer2Weights),
				ArrayUtils.Clone(network.inputLayerBiases),
				ArrayUtils.Clone(network.hiddenLayer1Biases),
				ArrayUtils.Clone(network.hiddenLayer2Biases),
				network.name
			);
		}

		private static double[] FeedNextLayer(double[] leftLayer, double[] rightLayer, double[,] weights, double[] biases)
		{
			double[] newRightLayer = ArrayUtils.Clone(rightLayer);
			for(int neuronIndex = 0; neuronIndex < rightLayer.Length; neuronIndex++)
			{
				double sum = 0;
