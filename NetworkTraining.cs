...
namespace NetworkTraining
{
    public static class Training
    {
        public static NeuralNetwork TrainNetwork(NeuralNetwork network, double learningRate, string labelsFilePath, string imagesFilePath)
        {
            ...
            for(int amountOfBatches = 0; amountOfBatches < 100; amountOfBatches++)
            {
                for(int batchIndex = 0; batchIndex < batchSize; batchIndex++)
                {
                    //Gets the new weights and biases, using the backpropagation function, still side effects on feedforward but idc right now
